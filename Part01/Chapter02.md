...
## 런레벨(run level)
0 ~ 6으로 이루어져있는 런레벨은 부팅의 마지막 단계에서 init 프로세스가 생성되면서, 이 init 프로세스가 참조하는 값이다. 이 런레벨에 따라 작동하는 서비스들을 조정할 수 있다.

|런레벨|기능|
|:-:|:-:|
|0|시스템 종료|
|1|root만 로그인 할 수 있는 단일 사용자 모드|
|2|네트워크 없는 다중 사용자 모드|
|3|CUI|
|4|미사용|
|5|GUI|
...
- `/etc/rc.d/init.d` 시스템 서비스의 시작 및 중지가 가능한 스크립트
- `rc` 실행 레벨에 따라 지정 서비스를 실행하는 스크립트
- `rc.local` rc.sysinit에 의해 실행되는 파일
- `rc.sysinit` init에 의해 실행되는 스크립트. 시스템을 초기화함
- `rc0.d ~ rc6.d` 각 레벨에 따른 초기화 스크립트.

## 로그인 로그아웃
- 리눅스는 X윈도우상에서, 콘솔상에서 로그인 및 로그아웃으로 나뉜다.
- 리눅스의 로그인 과정은 다음과 같다.
	1. `/etc/passwd` 필드를 확인한다.
	2. 셸 설정 파일을 실행한다. (터미널 실행, 환경변수 등록, 로그인메시지 출력 등 수행)
- 로그인 메시지 설정  

|설정파일|특징|
|:-:|:-:|
|/etc/issue|로컬 접속시 로그인 전 출력|
|/etc/issue.net|원격 접속시 로그인 전 출력|
|/etc/motd|로그인 성공시 출력|

- 로그아웃은 `logout`, `exit`, `[ctrl+D]`를 사용한다.
- 관리자는 장시간 행위가 없는 사용자를 `/etc/profile`에 `TMOUT`을 설정하여 강제 로그아웃 시킬 수 있다.
